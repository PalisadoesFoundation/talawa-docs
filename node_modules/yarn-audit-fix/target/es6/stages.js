...
import * as lf from './lockfile.js';
import { formatFlags, getNpm, getSymlinkType, getWorkspaces, getYarn, invoke, pkgDir, readJson, } from './util.js';
const __dirname = dirname(fileURLToPath(import.meta.url));
/**
 * Print runtime context digest.
 */
export const printRuntimeDigest = ({ temp, cwd, flags, manifest, }) => {
    var _a;
    if (flags.silent) {
        return;
    }
    ...
    const latestYafVersion = invoke(npmPath, ['view', 'yarn-audit-fix', 'version'], temp, true, false);
    const yafVersion = readJson(join(pkgDir(__dirname) + '', 'package.json')).version;
    // NOTE npm > 7.0.0 provides monorepo support
    if (isMonorepo && ((_a = semver.parse(npmVersion)) === null || _a === void 0 ? void 0 : _a.major) < 7) {
        console.warn("This project looks like monorepo, so it's recommended to use `npm v7` at least to process workspaces");
    }
    if (semver.gt(latestYafVersion, yafVersion)) {
        console.warn(`yarn-audit-fix version ${yafVersion} is out of date. Install the latest ${latestYafVersion} for better results`);
    }
 ...
 * @return {void}
 */
export const npmAuditFix = ({ temp, flags }) => {
 ...
 * @return {void}
 */
export const yarnInstall = ({ cwd, flags }) => {
    if (flags.dryRun) {
        return;
    }
    invoke(getYarn(), [
